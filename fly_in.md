...
- III. [Common Instructions](#chapter-iii-common-instructions) — 5
  - III.1 General Rules — 5
  - III.2 Makefile — 5
  - III.3 Additional Guidelines — 6
- IV. [Introduction](#chapter-iv-introduction) — 7
- V. [Constraints](#chapter-v-constraints) — 8
- VI. [Let the drone fly](#chapter-vi-let-the-drone-fly) — 9
- VII. [Mandatory Part](#chapter-vii-mandatory-part) — 11
...
- VIII. [Readme Requirements](#chapter-viii-readme-requirements) — 20
- IX. [Bonus Part](#chapter-ix-bonus-part) — 21
- X. [Submission and peer-review](#chapter-x-submission-and-peer-review) — 22
...
> ℹ️ The zones coordinates will always be integers, and there will always be a unique start and a unique end zone.
...
> ℹ️ It's highly recommended to make your own map files on top of the ones provided in the subject for handling edge cases and error handling.
...
> ℹ️ These secondary metrics are not mandatory to compute automatically, but learners are encouraged to display them in their simulation output or documentation to help peers evaluate performance.
...
> ℹ️ These benchmarks are provided as optimization targets to help you evaluate your algorithm's performance. Meeting these targets demonstrates a well-optimized implementation and will be assessed during peer evaluation.
...
**Figure VII.1:** Map easy 2 — a simple diamond topology: `start → junction → {priority zone, priority zone} → goal`, with 4 drones splitting across two parallel priority-zone branches.

**Figure VII.2:** Map medium 3 — `start` branches into a `fast_junction`/`fast_path` route and a `slow_path1`/`slow_path2` (restricted) route, which converge at `merge_point` before reaching a final zone.

**Figure VII.3:** Map hard 2 — a wider network from `city_hall` through multiple `gate`/`waiting_area` zones and parallel `priority_bypass` and `restricted` zone chains, converging at a `convergence` point before a `final_bottleneck` zone leading to `goal`.
...
> ⚠️ Place all your files at the root of your repository.
...
> ℹ️ Note that we may ask you to explain your code or possibly even to write some code. Make sure to be prepared for this.

> ℹ️ Evaluation maps may be different from the ones provided in the subject.
